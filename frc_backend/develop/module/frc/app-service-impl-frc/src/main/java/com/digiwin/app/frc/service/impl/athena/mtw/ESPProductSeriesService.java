...
import com.digiwin.app.frc.service.athena.mtw.service.IESPProductSeriesService;
 ...
 * @Description
 */
public class ESPProductSeriesService implements IESPProductSeriesService {

    @Autowired
    private ProductSeriesBiz productSeriesBiz;

    @Override
    public DWEAIResult postAddProductSeriesInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        //对参数进行解析并校验返回结果
        JSONArray dataContent =  parseDate(messageBody, true);
        List<JSONObject> result = productSeriesBiz.addProductSeries(dataContent);
        dataResult.put("product_info", result);
        return new DWEAIResult("0", "0", MultilingualismUtil.getLanguage("addSuccess"), dataResult);
    }

    @Override
    public DWEAIResult postDeleteProductSeriesInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        try {
            //对参数进行解析并校验返回结果
            JSONArray dataContent =  parseDate(messageBody, true);
            result = productSeriesBiz.deleteProductSeries(dataContent);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @Override
    public DWEAIResult postUpdateProductSeriesInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        //对参数进行解析并校验返回结果
        JSONArray dataContent = parseDate(messageBody, true);
        result = productSeriesBiz.updateProductSeries(dataContent);
    ...
    @Override
    public DWEAIResult getProductSeriesInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        try {
            //对参数进行解析返回结果
            JSONArray dataContent = parseDate(messageBody, false);
            List<JSONObject> result = productSeriesBiz.getProductSeries(dataContent);
            dataResult.put("product_info", result);
        } catch (Exception e) {
            e.printStackTrace();
