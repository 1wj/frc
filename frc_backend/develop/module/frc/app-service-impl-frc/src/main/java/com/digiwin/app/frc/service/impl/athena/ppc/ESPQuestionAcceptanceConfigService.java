...
import com.digiwin.app.frc.service.athena.ppc.service.IESPQuestionAcceptanceConfigService;
 ...
 * @Description
 */
public class ESPQuestionAcceptanceConfigService implements IESPQuestionAcceptanceConfigService {

    @Autowired
    private QuestionAcceptanceConfigBiz questionAcceptanceConfigBiz;

    @Override
    public DWEAIResult postAddQuestionAcceptanceConfigInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        //对参数进行解析并校验返回结果
        JSONArray dataContent =  parseDate(messageBody, true);
        List<JSONObject> result = questionAcceptanceConfigBiz.addQuestionAcceptanceConfig(dataContent);
        dataResult.put("acceptance_config_info", result);
        return new DWEAIResult("0", "0", MultilingualismUtil.getLanguage("addSuccess"), dataResult);
    }

    @Override
    public DWEAIResult postDeleteQuestionAcceptanceConfigInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        try {
            //对参数进行解析并校验返回结果
            JSONArray dataContent =  parseDate(messageBody, true);
            result = questionAcceptanceConfigBiz.deleteQuestionAcceptanceConfig(dataContent);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @Override
    public DWEAIResult postUpdateQuestionAcceptanceConfigInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        //对参数进行解析并校验返回结果
        JSONArray dataContent = parseDate(messageBody, true);
        result = questionAcceptanceConfigBiz.updateQuestionAcceptanceConfigInfo(dataContent);
    ...
    @Override
    public DWEAIResult getQuestionAcceptanceConfigInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        try {
            //对参数进行解析返回结果
            JSONArray dataContent = parseDate(messageBody, false);
            List<JSONObject> result = questionAcceptanceConfigBiz.getQuestionAcceptanceConfig(dataContent);
            dataResult.put("acceptance_config_info", result);
        } catch (Exception e) {
            e.printStackTrace();
