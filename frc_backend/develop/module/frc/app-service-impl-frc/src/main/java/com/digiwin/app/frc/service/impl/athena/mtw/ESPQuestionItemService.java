...
import com.digiwin.app.frc.service.athena.mtw.service.IESPQuestionItemService;
 ...
 * @Description
 */
public class ESPQuestionItemService implements IESPQuestionItemService {
    ...
    @Override
    public DWEAIResult postAddQuestionItemInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        //对参数进行解析并校验返回结果
        JSONArray dataContent =  parseDate(messageBody, true);
        List<JSONObject> result = questionItemBiz.addQuestionItem(dataContent);
        dataResult.put("item_info", result);
        return new DWEAIResult("0", "0", MultilingualismUtil.getLanguage("addSuccess"), dataResult);
    }

    @Override
    public DWEAIResult postDeleteQuestionItemInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        try {
            //对参数进行解析并校验返回结果
            JSONArray dataContent =  parseDate(messageBody, true);
            result = questionItemBiz.deleteQuestionItem(dataContent);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @Override
    public DWEAIResult postUpdateQuestionItemInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        //对参数进行解析并校验返回结果
        JSONArray dataContent = parseDate(messageBody, true);
        result = questionItemBiz.updateQuestionItem(dataContent);
    ...
    @Override
    public DWEAIResult getQuestionItemInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        try {
            //对参数进行解析返回结果
            JSONArray dataContent = parseDate(messageBody, false);
            List<JSONObject> result = questionItemBiz.getQuestionItem(dataContent);
            dataResult.put("item_info", result);
        } catch (Exception e) {
            e.printStackTrace();
