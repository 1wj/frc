...
import com.digiwin.app.frc.service.athena.mtw.service.IESPDefectCodeService;
 ...
 * @Description
 */
public class ESPDefectCodeService implements IESPDefectCodeService {
    ...
    @Override
    public DWEAIResult postAddDefectCodeInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        //对参数进行解析并校验返回结果
        JSONArray dataContent =  parseDate(messageBody, true);
        List<JSONObject> result = defectCodeBiz.addDefectCodeInfo(dataContent);
        dataResult.put("defect_info", result);
        return new DWEAIResult("0", "0", MultilingualismUtil.getLanguage("addSuccess"), dataResult);
    }


    @Override
    public DWEAIResult postDeleteDefectCodeInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        try {
            //对参数进行解析并校验返回结果
            JSONArray dataContent =  parseDate(messageBody, true);
            result = defectCodeBiz.deleteDefectCode(dataContent);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @Override
    public DWEAIResult postUpdateDefectCodeInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        //对参数进行解析并校验返回结果
        JSONArray dataContent = parseDate(messageBody, true);
        result = defectCodeBiz.updateDefectCode(dataContent);
    ...
    @Override
    public DWEAIResult getDefectCodeInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        try {
            //对参数进行解析返回结果
            JSONArray dataContent = parseDate(messageBody, false);
            List<JSONObject> result = defectCodeBiz.getDefectCode(dataContent);
            dataResult.put("defect_info", result);
        } catch (Exception e) {
            e.printStackTrace();
