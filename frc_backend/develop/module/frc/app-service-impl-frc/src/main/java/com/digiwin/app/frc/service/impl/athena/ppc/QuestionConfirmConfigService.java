...
import com.digiwin.app.frc.service.athena.ppc.service.IQuestionConfirmConfigService;
 ...
 * @Description
 */
public class QuestionConfirmConfigService implements IQuestionConfirmConfigService {
    ...
    public DWServiceResult addQuestionConfirmConfigInfo(String messageBody) throws Exception {
        ...
        try {
            ...
            List<JSONObject> resultInfo = questionConfirmConfigBiz.addQuestionConfirmConfig(dataContent);
    ...
    @Override
    public DWServiceResult deleteQuestionConfirmConfigInfo(String messageBody) throws Exception {
        DWServiceResult result = new DWServiceResult();
        try {
            JSONArray dataContent = parseDate(messageBody, true);
            boolean resultInfo = questionConfirmConfigBiz.deleteQuestionConfirmConfig(dataContent);
            if (resultInfo) {
                result.setSuccess(true);
    ...
    @Override
    public DWServiceResult updateQuestionConfirmConfigInfo(String messageBody) throws Exception {
        DWServiceResult result = new DWServiceResult();
        try {
            //对参数进行解析并校验返回结果
            JSONArray dataContent = parseDate(messageBody, true);
            boolean resultInfo = questionConfirmConfigBiz.updateQuestionConfirmConfigInfo(dataContent);
            if (resultInfo) {
                result.setSuccess(true);
    ...
    @Override
    public DWServiceResult getQuestionConfirmConfigInfo(String messageBody) throws Exception {
        DWServiceResult result = new DWServiceResult();
        try {
            //对参数进行解析返回结果
            JSONArray dataContent = parseDate(messageBody, false);
            List<JSONObject> resultInfo = questionConfirmConfigBiz.getQuestionConfirmConfig(dataContent);
            result.setData(resultInfo);
            result.setSuccess(true);
