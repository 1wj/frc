...
import com.digiwin.app.frc.service.athena.ppc.service.IESPQuestionConfirmConfigService;
 ...
 * @Description
 */
public class ESPQuestionConfirmConfigService implements IESPQuestionConfirmConfigService {


    @Autowired
    private QuestionConfirmConfigBiz questionConfirmConfigBiz;


    @Override
    public DWEAIResult postAddQuestionConfirmConfigInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        //对参数进行解析并校验返回结果
        JSONArray dataContent =  parseDate(messageBody, true);
        List<JSONObject> result = questionConfirmConfigBiz.addQuestionConfirmConfig(dataContent);
        dataResult.put("confirm_config_info", result);
        return new DWEAIResult("0", "0", MultilingualismUtil.getLanguage("addSuccess"), dataResult);
    }

    @Override
    public DWEAIResult postDeleteQuestionConfirmConfigInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        try {
            //对参数进行解析并校验返回结果
            JSONArray dataContent =  parseDate(messageBody, true);
            result = questionConfirmConfigBiz.deleteQuestionConfirmConfig(dataContent);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @Override
    public DWEAIResult postUpdateQuestionConfirmConfigInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        //对参数进行解析并校验返回结果
        JSONArray dataContent = parseDate(messageBody, true);
        result = questionConfirmConfigBiz.updateQuestionConfirmConfigInfo(dataContent);
    ...
    @Override
    public DWEAIResult getQuestionConfirmConfigInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        try {
            //对参数进行解析返回结果
            JSONArray dataContent = parseDate(messageBody, false);
            List<JSONObject> result = questionConfirmConfigBiz.getQuestionConfirmConfig(dataContent);
            dataResult.put("confirm_config_info", result);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private JSONArray parseDate(String messageBody, boolean check) {
        ...
        JSONArray dataContent = (JSONArray) parameter.get("confirm_config_info");
        if (check) {
