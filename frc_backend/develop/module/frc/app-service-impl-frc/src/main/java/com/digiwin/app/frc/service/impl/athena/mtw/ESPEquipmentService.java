...
import com.digiwin.app.frc.service.athena.mtw.service.IESPEquipmentService;
 ...
 * @Description
 */
public class ESPEquipmentService implements IESPEquipmentService {

    @Autowired
    private EquipmentBiz equipmentBiz;

    @Override
    public DWEAIResult postAddEquipmentInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        //对参数进行解析并校验返回结果
        JSONArray dataContent =  parseDate(messageBody, true);
        List<JSONObject> result = equipmentBiz.addEquipment(dataContent);
        dataResult.put("workstation_info", result);
        return new DWEAIResult("0", "0", MultilingualismUtil.getLanguage("addSuccess"), dataResult);
    }

    @Override
    public DWEAIResult postDeleteEquipmentInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        try {
            //对参数进行解析并校验返回结果
            JSONArray dataContent =  parseDate(messageBody, true);
            result = equipmentBiz.deleteEquipment(dataContent);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @Override
    public DWEAIResult postUpdateEquipmentInfo(Map<String, Object> headers, String messageBody) throws Exception {
        boolean result;
        //对参数进行解析并校验返回结果
        JSONArray dataContent = parseDate(messageBody, true);
        result = equipmentBiz.updateEquipment(dataContent);
    ...
    @Override
    public DWEAIResult getEquipmentInfo(Map<String, Object> headers, String messageBody) throws Exception {
        Map<String, Object> dataResult = new HashMap<>(16);
        try {
            //对参数进行解析返回结果
            JSONArray dataContent = parseDate(messageBody, false);
            List<JSONObject> result = equipmentBiz.getEquipment(dataContent);
            dataResult.put("workstation_info", result);
        } catch (Exception e) {
            e.printStackTrace();
